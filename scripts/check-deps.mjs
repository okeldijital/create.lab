 ...
 * BUILD-008 adds Infrastructure → Capacity repository adapters.
 */
...
const ALLOWED_DEPS = {
  ...
  infrastructure: new Set(["application", "capacity", "organization", "crm", "workforce", "core", "config"]),
...
function main() {
  ...
  for (const name of packages) {
    if (!REGISTERED.has(name)) { console.error(`Unregistered package directory: packages/${name}`); failed = true; continue; }
    const pkg = readPackageJson(name); if (!pkg) { console.error(`Missing package.json for ${name}`); failed = true; continue; }
    const allowed = ALLOWED_DEPS[name]; const actual = collectWorkspaceDeps(pkg); graph.set(name, new Set(actual));
    for (const dep of actual) if (!REGISTERED.has(dep) || !allowed.has(dep)) { console.error(`Dependency violation: ${name} → ${dep} is not allowed by the matrix.`); failed = true; }
  }
  const visiting = new Set(); const visited = new Set();
  function dfs(node, path) { if (visiting.has(node)) { console.error(`Circular dependency detected: ${[...path, node].join(" → ")}`); failed = true; return; } if (visited.has(node)) return; visiting.add(node); for (const next of graph.get(node) || []) dfs(next, [...path, node]); visiting.delete(node); visited.add(node); }
  for (const name of graph.keys()) dfs(name, []);
  if (failed) process.exit(1); console.log("Dependency graph check passed.");
}
main();
